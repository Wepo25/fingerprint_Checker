...
public class Fingerprint {
  ...
  public static boolean[] getNeighbours(boolean[][] image, int row, int col) {
    ...
    int[] rowrotations = { -1, -1, 0, 1, 1, 1, 0, -1 };
    int[] colrotations = { 0, 1, 1, 1, 0, -1, -1, -1 };
    boolean[] value = new boolean[8];
    for (int i = 0; i < 8; i++) {
      try {
        value[i] = image[row + rowrotations[i]][col + colrotations[i]];
      } catch (Exception e) {
        value[i] = false;
      }
    }
  ...
  public static int blackNeighbours(boolean[] neighbours) {
    int newblackneighbours = 0;
    for (int i = 0; i < neighbours.length; i++) {
      if (neighbours[i] == true) {
        newblackneighbours++;
      }
    }
    return newblackneighbours;
  }
  ...
  public static boolean[][] thinningStep(boolean[][] image, int step) {
    ...
    for (int i = 0; i < image.length; i++) {
      for (int j = 0; j < image[i].length; j++) {
        boolean[] tempneighbours = getNeighbours(image, i, j);
        Integer tempblackNeighbours = blackNeighbours(getNeighbours(image, i, j));
        boolean A = (image[i][j] == true && tempblackNeighbours != null && 2 <= tempblackNeighbours
            && tempblackNeighbours <= 6 && transitions(tempneighbours) == 1);
        boolean B = (tempneighbours[0] == false || tempneighbours[2] == false || tempneighbours[var2] == false);
        boolean C = (tempneighbours[var1] == false || tempneighbours[4] == false || tempneighbours[6] == false);
        if (A && B && C) {
          newimage[i][j] = false;
        } else {
    ...
    if (identical(image, newimage) == false && step == 0) {
      return thinningStep(newimage, 1);
    } else {
      return newimage;
    }
  }
  ...
  public static boolean[][] thin(boolean[][] image) {
    boolean[][] image1 = image;
    boolean[][] image2 = thinningStep(image1, 0);
    while (identical(image1, image2) == false) {
      return thin(image2);
    }
    return image2;
  }
  ...
  public static boolean[][] connectedPixels(boolean[][] image, int row, int col, int distance) {
    boolean[][] a = new boolean[image.length][image[0].length];
    a[row][col] = true;
    boolean pixelchange;
    do {
      pixelchange = false;
      for (int i = 0; i < image.length; ++i) {
        for (int j = 0; j < image[i].length; ++j) {
          ...
          if (!((row - distance <= i) && (i <= row + distance)) || !((col - distance <= j) && (j <= col + distance))) {
            continue;
          }
          try {
            if (blackNeighbours(getNeighbours(a, i, j)) > 0) {
              if (a[i][j] == false) {
                a[i][j] = true;
                pixelchange = true;
              }
  ...
  public static int[] applyRotation(int[] minutia, int centerRow, int centerCol, int rotation) {
    ...
    int newX = (int) (x * Math.cos(radRotation) - y * Math.sin(radRotation));
    int newY = (int) (x * Math.sin(radRotation) + y * Math.cos(radRotation));
    int newRow = centerRow - newY;
    int newCol = newX + centerCol;
    int newOrientation = (minutia[2] + rotation) % 360;
    int[] rotatedMinutia = { newRow, newCol, newOrientation };
  ...
  public static int[] applyTransformation(int[] minutia, int centerRow, int centerCol, int rowTranslation,
    ...
    int[] tempMinutia = applyRotation(minutia, centerRow, centerCol, rotation);
    int[] transformedMinutia = applyTranslation(tempMinutia, rowTranslation, colTranslation);
  ...
  public static List<int[]> applyTransformation(List<int[]> minutiae, int centerRow, int centerCol, int rowTranslation,
    ...
    for (int i = 0; i < minutiae.size(); i++) {
      minutiae.set(i,
          applyTransformation(minutiae.get(i), centerRow, centerCol, rowTranslation, colTranslation, rotation));
    }
    return minutiae;
  }
  ...
  public static int matchingMinutiaeCount(List<int[]> minutiae1, List<int[]> minutiae2, int maxDistance,
    ...
    for (int i = 0; i < minutiae1.size(); i++) {
      for (int j = 0; j < minutiae2.size(); j++) {
        boolean sameOrientation = Math.abs(minutiae1.get(i)[2] - minutiae2.get(j)[2]) <= maxOrientation;
        double tempRow = Math.pow((minutiae1.get(i)[0] - minutiae2.get(j)[0]), 2);
        double tempCol = Math.pow((minutiae1.get(i)[1] - minutiae2.get(j)[1]), 2);
        boolean sameDistance = Math.sqrt(tempRow + tempCol) <= maxDistance;
        if (sameOrientation && sameDistance) {
          totalOverlap++;
        }
      }
    }
  ...
  public static boolean match(List<int[]> minutiae1, List<int[]> minutiae2) {
    int identicalMinutiae = 0;
    int maxDistance = DISTANCE_THRESHOLD;
    int maxOrientation = ORIENTATION_THRESHOLD;
    int centerRow, centerCol, rowTranslation, colTranslation, rotation;
    for (int i = 0; i < minutiae1.size(); i++) {
      centerRow = minutiae1.get(i)[0];
      centerCol = minutiae1.get(i)[1];
      for (int j = 0; j < minutiae2.size(); j++) {
        rowTranslation = minutiae2.get(j)[0] - minutiae1.get(i)[0];
        colTranslation = minutiae2.get(j)[1] - minutiae1.get(i)[1];
        rotation = minutiae2.get(j)[2] - minutiae1.get(i)[2];
        for (int k = rotation - MATCH_ANGLE_OFFSET; k <= rotation + MATCH_ANGLE_OFFSET; k++) {
          List<int[]> tempMinutiae2 = applyTransformation(minutiae2, centerRow, centerCol, rowTranslation,
              colTranslation, rotation);
          identicalMinutiae += matchingMinutiaeCount(minutiae1, tempMinutiae2, maxDistance, maxOrientation);
        }
      }
    }
    return identicalMinutiae >= FOUND_THRESHOLD;
  }
}